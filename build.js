...
/***/ function(module, exports, __webpack_require__) {
	...
	document.getElementById('inp').onkeyup = function (e) {
	  ...
	  var msg = message.split('').map(codify);
	  bridge.send(text(msg));
	};
...
/***/ function(module, exports) {
	...
	function defaultWriter(id, data) {
	  writeQueue.push([id].concat(_toConsumableArray(data)));
	}
	...
	function lengthOfMessage(message) {
	  return 1 + message.data.length;
	}

	function scheduler(_ref10) {
	  var write = _ref10.write;
	  var writable = _ref10.writable;
	  ...
	  return {
	    ...
	    tick: function tick() {
	      if (writeQueue.length > 0 && writable()) {
	        var toBeWritten = [];
	        while (writeQueue.length > 0 && lengthOfMessage(writeQueue[0]) <= USABLE_BUFFER_SIZE - toBeWritten.length) {
	          var _writeQueue$shift = writeQueue.shift();

	          var id = _writeQueue$shift.id;
	          var data = _writeQueue$shift.data;

	          toBeWritten = toBeWritten.concat([id].concat(_toConsumableArray(data)));
	        }
	        write(toBeWritten);
	      }
	    }
