...
function btn(i)
  return band(2^i, current_controller) ~= 0
end
...
player = 
{
  ...
  dump=function(this)
    return player.dump_type.serialize{
      this.p_jump,
      this.p_dash,
      this.grace,
      this.jbuffer,
      ...
      this.dash_target.x, this.dash_target.y,
      this.dash_accel.x, this.dash_accel.y,
      this.spr_off,
      this.was_on_ground,
      this.x,
      this.y,
  ...
  restore=function(this, arg)
    local des = player.dump_type.deserialize(arg, 1)[1]
    this.p_jump = des[1]
    this.p_dash = des[2]
    this.grace = des[3]
    this.jbuffer = des[4]
    this.djump = des[5]
    this.dash_time = des[6]
    this.dash_effect_time = des[7]
    this.dash_target.x = des[8]
    this.dash_target.y = des[9]
    this.dash_accel.x = des[10]
    this.dash_accel.y = des[11]
    this.spr_off = des[12]
    this.was_on_ground = des[13]
    this.x = des[14]
    this.y = des[15]
    this.rem.x = des[16]
    this.rem.y = des[17]
  ...
  if this.dash_time > 0 then
   init_object(smoke,this.x,this.y)
      ...
      this.y = -100 -- just freeze the player
      has_completed_room = true
    end
...
player_spawn = {
  ...
  dump=function(this)
    return player_spawn.dump_type.serialize({
      this.x,
      this.y,
      this.target.x,
      this.target.y,
  ...
  restore=function(this, arg)
    local des = player_spawn.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
    this.target.x = des[3]
    this.target.y = des[4]
...
spring = {
  ...
  dump=function(this)
    return spring.dump_type.serialize{
      this.x, this.y, this.hide_in, this.hide_for, this.spr, this.delay
    }
  end,
  restore=function(this, arg)
    local des = spring.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
    this.hide_in = des[3]
    this.hide_for = des[4]
...
balloon = {
  id=6,
	tile=22,
	init=function(this) 
		this.offset=balloon_rng(level_index(), curballoon)
    curballoon += 1
		this.start=this.y
		this.timer=0
  ...
  dump=function(this)
    return balloon.dump_type.serialize({
      this.x, this.y, this.offset, this.start, this.timer, this.spr
    })
  end,
  restore=function(this, arg)
    local des = balloon.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
    this.offset = des[3]
    this.start = des[4]
...
fall_floor = {
  ...
  dump=function(this)
    return fall_floor.dump_type.serialize({
      this.x, this.y, this.state, this.solid, this.delay, this.collideable
    })
  end,
  restore=function(this, arg)
    local des = fall_floor.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
    this.state = des[3]
    this.solid = des[4]
    this.delay = des[5]
    this.collideable = des[6]
  end,
	update=function(this)
		-- idling
		if this.state == 0 then
...
fruit={
  ...
  dump=function(this)
    return fruit.dump_type.serialize{
      this.x, this.y,
      this.off,
      this.start
    }
  ...
  restore=function(this, arg)
    local des = fruit.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
    this.off = des[3]
    this.start = des[4]
...
fly_fruit={
  ...
  dump=function(this)
    return fly_fruit.dump_type.serialize{
      this.x,
      this.y,
      this.start,
      this.fly,
  ...
  restore=function(this, arg)
    local des = fly_fruit.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
    this.start = des[3]
    this.fly = des[4]
...
fake_wall = {
  ...
  dump=function(this)
    return fake_wall.dump_type.serialize{
      this.x,
      this.y
    }
  end,
  restore=function(this,arg)
    local des = fake_wall.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
  end,
	update=function(this)
...
key={
  ...
  dump=function(this)
    return key.dump_type.serialize{
      this.x,
      this.y,
      this.spr
    }
  ...
  restore=function(this,arg)
    local des = key.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
    this.spr = des[3]
  end,
...
chest={
  ...
  dump=function(this)
    return chest.dump_type.serialize{
      this.x, this.y,
      this.start, this.timer
    }
  end,
  restore=function(this, arg)
    local des = chest.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
    this.start = des[3]
    this.timer = des[4]
	...
	update=function(this)
		if has_key then
			this.timer-=1
			this.x=this.start-1+chestrnd(3)
...
platform={
  ...
  dump=function(this)
  return platform.dump_type.serialize{
    this.x, this.y,
    this.dir,
    this.spd.x, this.spd.y,
    this.rem.x, this.rem.y,
    this.last
  }
  end,
  restore=function(this, arg)
    local des = platform.dump_type.deserialize(arg, 1)[1]
    this.x = des[1]
    this.y = des[2]
    this.dir = des[3]
    this.spd.x = des[4]
...
function load_room(x,y)
  ...
  objects = {}
	...
	room.x = x
	room.y = y
  max_djump = level_index() > 21 and 2 or 1
...
functions = {}
functions[0] = { -- loadlevel
	 input={
	   byte,
	 },
	 output={},
	 execute=function(args)
     load_room(args[1] % 8, flr(args[1] / 8))
	 	 return {}
	 end
}

frames_to_advance = {}
functions[1] = { -- nextframe
  input={ byte },
  output={
    boolean,
    number,
...
functions[2] = { -- dumpstate
  input={},
  output={opacify(array(tuple({byte, array(byte)})))},
  execute=function()
    local result = {}
    for o in all(objects) do
      if o.type.dump then
        add(result, {o.type.id, o.type.dump(o)})
      end
    end
    return {
      result
...
type_ids = {}
type_ids[0] = player
type_ids[1] = fake_wall
type_ids[2] = fruit
type_ids[3] = player_spawn
type_ids[4] = spring
type_ids[5] = fall_floor
type_ids[6] = balloon
type_ids[7] = key
type_ids[8] = chest
type_ids[9] = fly_fruit
type_ids[10] = platform

functions[3] = { -- pushstate
  input={opacify(array(tuple({byte, array(byte)})))},
  output={},
  execute=function(args)
    local entities = args[1]
    objects = {}
    for e in all(entities) do
      local type = type_ids[e[1]]
      local obj = init_object(type, 0, 0)
      type.restore(obj, e[2])
    end
    return {}
  end
}

functions[4] = { -- shownumber
  input={number},
  output={},
  execute=function(args)
    printh(args[1])
...
functions[5] = { -- playback
  input={array(byte)},
  output={},
  execute=function(args)
    load_room(room.x,room.y)
    frames_to_advance = {}
    for f in all(args[1]) do
      add(frames_to_advance, f)
    end
    return {}
  end
}
...
function _update()
  ...
  -- ????? dont know why we need this header ~= 0
  if header == 3 then
    poke(0x5f80, 2)
    for i = 0, 126 do
  ...
  if #write_queue > 0 and band(header, 1) == 0 then
    poke(0x5f80, 5)
    -- this might be real slow
    for i = 1, 127 do
      local msg = 'writing spot '..i..' '
      if #write_queue > 0 then
        poke(0x5f80 + i, write_queue[1])
        msg = msg..'yes, '..peek(0x5f80 + i)
  ...
  if #frames_to_advance > 0 then
    current_controller = frames_to_advance[1]
    del(frames_to_advance, current_controller)
    __update()
  end
end
